...
| SK10 | Archive | Opens close/archive proof posture plus reviewed archive authority metadata until live archive controls exist. | wired | Spark Archive opens Session Close Archive Proof from `/bridge/session-close-archive-proof`; it is display-only typed state and now also surfaces reviewed archive catalog, reload/run-again posture, transcript-access mode, and archive metadata while still not closing, archiving, deleting, reloading, running again, replaying, POSTing, calling `/bridge/message`, or exposing raw prompt/worker chat/session history/log/detail bodies. Orchestrator intake remains compact typed session state only; raw detail is fetched on demand only. |
...
| SK13 | Routines | Opens current runtime continuity status plus reviewed routine authority posture until executable automation exists. | wired | Spark Routines opens backend-sourced Goal Runtime, Routine Authority, and Workflow Dispatch Status; it is display-only typed state and does not run automation, mutate schedulers, paste raw logs/transcripts/details, or self-approve. |
...
| ROU1 | Routine list | Shows configured routines for active project/system. | wired | Spark Routines renders a display-only Routine list frame from `/bridge/routines`, surfacing reviewed routine ids, names, owners, scope refs, trigger labels, created-at metadata, evidence refs, and enabled/disabled state for the active project/system without synthesizing fake routines or exposing automation control authority. |
...
| ROU3 | Enable/disable routine | Toggles routine active state. | wired | Spark Routines renders a display-only Routine control posture plus real enabled/disabled state visibility from `/bridge/routines`, surfacing reviewed toggle posture while keeping enable/disable controls unavailable and exposing no active-state mutation route or automation toggle backend. |
| ROU4 | Routine trigger | Supports manual run-now once routine execution exists. | planned | Run creates a visible event/result. |
| ROU5 | Cadence/trigger view | Shows schedule, heartbeat, or event trigger. | wired | Spark Routines renders a display-only Cadence/trigger view from `/bridge/routines`, surfacing reviewed trigger kinds, trigger labels, and cadence/manual routine posture while keeping scheduler mutation, next-run prediction, and routine execution unavailable. |
...
| ROU9 | Prime-owned routine review | Prime reviews routine outputs and only escalates meaningful user gates. | wired | Spark Routines renders a display-only Prime routine review summary/posture plus Prime routine action posture from `/bridge/routines`, surfacing reviewed disposition/source, proof trail, gate-required/escalation-required posture, and action-authorized false sentinels from reviewed routine authority so Prime-owned routine review remains visible without exposing accept/reroute/retry/escalate execution or scheduler control. |
...
| ARC1 | Session archive list | Shows archived sessions that can be inspected later. | wired | Spark Archive renders a real display-only Session archive list from `/bridge/session-close-archive-proof`, showing reviewed archive ids, session ids/names, archived status/time, reload/run-again availability, transcript-access mode, and proof refs without inventing archive rows, replay controls, or durable archive mutation. |
| ARC2 | Reload session | Restores an archived session into an active/reopened state where supported. | wired | Spark Archive renders a display-only Archive reload posture from `/bridge/session-close-archive-proof`, surfacing reviewed reload plan id, status, blockers, proof refs, and reload-authorized posture while keeping execution unauthorized and exposing no reopen/reload action. |
| ARC3 | Run again | Allows rerun/resume/restart from archived session context where backend supports it. | wired | Spark Archive renders a display-only Archive run-again posture from `/bridge/session-close-archive-proof`, surfacing reviewed run-again plan id, status, blockers, proof refs, and run-again-authorized posture while keeping execution unauthorized and exposing no rerun/resume/restart action. |
| ARC4 | Archive metadata | Stores project, model/backend, role, timestamps, status, and source session id. | wired | Spark Archive renders display-only Archive metadata from `/bridge/session-close-archive-proof`, surfacing archive id, target session, session name, final status, archived time, close request id, intended action, write-through completion, Obsidian capture completion, and recoverable-session posture without opening a transcript body or mutating archive state. |
| ARC5 | Context reference | Allows Prime/session to reference archived context intentionally. | wired | Spark Archive renders a display-only Archive context reference frame from `/bridge/session-close-archive-proof`, surfacing explicit archive/session/proof refs only so archived context can be referenced intentionally without hidden prompt drag, transcript body exposure, or backend context mutation. |
...
| ARC8 | Transcript access | Allows full transcript access when available and authorized. | wired | Spark Archive renders a display-only Transcript access posture from `/bridge/session-close-archive-proof`, surfacing reviewed handle id, authorization state, transcript-access mode, transcript hash/length posture, and proof refs while keeping full transcript body unavailable and on-demand only. |
...
| ARC10 | Restore proof/artifacts | Links archived session to proof, files, or artifacts created. | wired | Spark Archive renders display-only Restore proof/artifacts from `/bridge/session-close-archive-proof`, surfacing reviewed archive proof refs plus reload/run-again plan proof refs and transcript-access proof refs as inspectable ids without opening raw artifact bodies or executing restore actions. |
...
| VOC10 | Voice command intents | Shows preview-only recognized command families such as reset, reload, open, close, filter, and crosscheck before any executable voice route exists. | wired | Voice I/O renders a display-only Voice intent summary plus Voice command preview boundary from `/bridge/voice-io`, surfacing reviewed VOC10 command family, action, target ref, confidence, confirmation posture, and non-executable command coverage from `meridian_core.voice_io` so recognized voice-command intent is visible without capturing audio, executing commands, submitting prompts, reading aloud, muting output, or exposing raw prompt/response/history. |
...
| HMS4 | Prime review path | Prime reviews harness intent, risk, and proof needs before model interaction. | wired | Prime Runtime Logic renders a display-only `Prime review before dispatch` section from `/bridge/prime-logic`, naming intent, reviewed action, owner, risk, Prime status, Aegis proof needs, and blocking proof gates while leaving route/provider/payload decisions with Relay and Model Harness. |
