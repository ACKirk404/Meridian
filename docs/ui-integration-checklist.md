...
| MOD9A | Per-call GOAL / Intent | Shows the dispatch-scoped goal for a model call when Relay exposes it. | planned | Displays backend-owned call goal, action type, expected output shape, risk tier, proof requirement, disallowed outputs, payload budget ref, and evidence refs; does not infer user intent from transcript text or create provider routes. |
